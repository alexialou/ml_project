...
outpath = '/data/Dropbox/Projects/Ash_Chen/clerkships/functional_NYU_judge/output/'
clerk_version = 1
fw_pkl = '/data/Dropbox/Projects/Ash_Chen/clerkships/functional_NYU_judge/ref/function_words.pkl'
...
def feature_two_grams(sentences_dir, out_path, clerk_path, meta_path, fws, case_year=-1):
    ...
    for filename in glob.iglob(files, recursive=True):
        ...
        if songer_names.count() > 0:
            ...
            '''
            entry = [case_year, func_word_freq(filename, fws), songer_name]
            data_list.append(entry)
    parsed_data = pd.DataFrame(data = data_list, columns = cols)
    if case_year != -1:
        parsed_data.to_csv(out_path + repr(case_year) + '_parsed_functional.csv', index=False)
    else:
